...
- THEN the detail panel shows metrics and employee list for that node's subtree (single employee if leaf)
- AND the node is highlighted in the tree

### Requirement: Per-node aggregated metrics panel

Upon selecting any area/subarea node, the system SHALL compute and display per-phase metrics from fixture data:

| Phase (`CyclePhase`) | Metric | Computation | Visual |
|---|---|---|---|
| `medio-anio` | Avg goal progress (%) | `mean(goals.progress)` of all employees in subtree (only employees with ≥1 goal) | `ProgressIndicator` + badge |
| `medio-anio` | Completed goals | Count of `goals` where `progress ≥ targetValue` in subtree | Badge-success with count |
| `medio-anio` | Pending goals | Count of `goals` where `progress < targetValue` or null in subtree | Badge-warning with count |
| `fin-anio` | Avg evaluation rating (1–5) | `mean(rhRating)` across all `CompetencyRating` in subtree | Numeric badge + star |
| `inicio-anio` | — | Metrics not applicable | EmptyState "Métricas no disponibles en inicio de año" |

Employees with zero goals or zero competency ratings SHALL be excluded from means. Empty subtrees SHALL show "—" (not zero).

#### Scenario: Mid-year metrics for an area with goals

- GIVEN `cyclePhase = 'medio-anio'`, RRHH selects node with 7 employees (3 with goals)
...
- AND the 4 employees with no goals are excluded from the mean

#### Scenario: End-of-year metrics for an area

- GIVEN `cyclePhase = 'fin-anio'`, RRHH selects a subarea with 5 employees
- WHEN detail panel renders
- THEN avg rating = mean of all `rhRating` values across all competencies for those 5 employees
...
The system SHALL render a read-only table of all employees belonging to the selected node's subtree. Columns: name, position (from `OrgNode`), profile (label in Spanish). The list SHALL be sorted alphabetically by name. Rows SHALL NOT be clickable (no drill-down).
...
- [ ] Employee table shows all subtree employees with name, position, profile.
