...
- BD: migración `evaluation_goals` (+2 columnas FLOAT NULL); backfill por fase activa: si `cycles.current_phase=avance` → `avance_progress=current_value`, si `cierre` → `cierre_progress=current_value` (no siempre avance).
- Separación por fase: metas/ratings y comentarios totalmente separados por fase; editable solo fase actual, fase previa inmutable. El snapshot de cierre usa la fase activa, no `finished_at` (sin contradicción con el change que quitó `finished_at`).
- 9-box por fase: snapshot avance usa evaluaciones de avance, snapshot cierre usa evaluaciones de cierre (inmutable por fase).
- Código es verdad para `EvaluationDetail` y permisos de jefa: la spec no redefine esos contratos, solo los snapshots descritos aquí.
...
- Sin cambios de ponderación de metas ni de catálogo de pilares; sin triggers email; sin migración de `final_rating` histórico; sin reintroducir `finished_at`; sin redefinir `EvaluationDetail`/permisos jefa.
