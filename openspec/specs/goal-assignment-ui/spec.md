...
- GIVEN `cyclePhase === 'inicio-anio'` y categoría con al menos una meta hija
...
#### Scenario: Sin botón eliminar categoría fuera de inicio-anio
...
- THEN el formulario inline muestra `alert-error` "Ya existe una categoría con ese nombre."
...
| `CategoryCard` | Card DaisyUI con header (nombre, peso, badge de suma de metas), tabla de `GoalRow`, botón "Nueva meta" (modo editor). Incluye formularios inline para crear/editar categorías cuando está en modo edición. |
| `GoalRow` | Fila de meta: nombre, unidad, peso, `targetValue`, `KpiBadge[]`, acciones. Acciones condicionales: editor → Editar/Eliminar; lector → "Solicitar cambio". En modo edición, las celdas se transforman en inputs inline con checkboxes para KPIs y `CustomSelect` para unidad. |
...
| Nombre de categoría único por asignación | `addCategory` / `updateCategory` | `alert-error` en formulario inline |
| `targetValue > 0` | submit del formulario inline | `alert-error` |
