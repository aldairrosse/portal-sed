...
export async function importarComentariosJSON(
  file: File,
  ...
  if (
    ...
    parsed === null ||
    !("comentarios" in parsed)
  ) {
    return {
      ok: false,
      error: 'Estructura inválida: falta la propiedad "comentarios".',
    };
  }
  ...
  for (const [key, val] of Object.entries(comentarios)) {
    if (!/^\d+$/.test(key)) {
      return {
        ok: false,
        error: `Clave "${key}" no es un ID numérico válido.`,
      };
    }
    if (!Array.isArray(val)) {
      return {
        ok: false,
        error: `La clave "${key}" debe contener un arreglo.`,
      };
    }
