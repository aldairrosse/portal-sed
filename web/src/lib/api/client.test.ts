...
describe("client.ts", () => {
  ...
  describe("fetch interceptor", () => {
    ...
    it("sets credentials: include on requests", async () => {
      ...
      await (client as unknown as { GET(url: string): Promise<unknown> }).GET(
        "/test",
      );
    ...
    it("redirects to /login and saves return_to on SSO_SESSION_EXPIRED 401", async () => {
      ...
      mockFetch.mockResolvedValue(
        new Response(
          JSON.stringify({ error: { code: "SSO_SESSION_EXPIRED" } }),
          {
            status: 401,
      ...
      await (
        clientMod as unknown as { GET(url: string): Promise<unknown> }
      ).GET("/test");

      expect(getAssignedHref()).toBe("/login");
      expect(sessionStorage.getItem("return_to")).toBe(
