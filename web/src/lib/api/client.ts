  ...
  if (response.status === 401) {
    try {
      ...
      if (body?.error?.code === "SSO_SESSION_EXPIRED") {
        ...
        try {
          const devRes = await fetch("/api/v1/dev/status", {
            credentials: "include",
          });
        ...
        console.warn(
          "[sso] 401 SSO_SESSION_EXPIRED -> redirect /login",
  ...
  if (response.status === 403) {
    try {
      ...
      if (body?.error?.code === "OTP_REQUIRED") {
        console.warn(
          "[sso] 403 OTP_REQUIRED -> redirect step-up",
          window.location.pathname,
        );
        window.location.href =
          "/api/v1/auth/sso-step-up?return_to=" +
