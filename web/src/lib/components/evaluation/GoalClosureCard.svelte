			...
			<div class="flex items-start justify-between gap-2 mb-3">
				<div class="flex-1 min-w-0">
					<div class="flex flex-wrap items-center gap-1">
						...
						<span
							class="badge badge-ghost badge-xs inline-flex items-center gap-1"
							>{#if goal.direction === 'descendente'}<ArrowDown
									size={12}
								/>{:else}<ArrowUp size={12} />{/if}Objetivo: {formatFinal(
								goal.targetValue,
							)}</span
						>
						{#if goal.direction === 'descendente' && goal.baselineValue != null}<span
								class="badge badge-ghost badge-xs"
								>Inicio: {formatFinal(goal.baselineValue)}</span
							>{/if}
				...
				{#if mode === 'self' && canEdit && !selfReadMode}
					<label class="label px-0 py-1" for={progressId}>
						<span class="label-text text-xs"
							>{isAvance ? 'Avance actual' : 'Avance final'}</span
						>
					</label>
					<div class="flex items-center gap-2">
						<input
							...
							aria-label="{isAvance
								? 'Avance actual'
								: 'Avance final'} de {goal.name}"
						/>
						<ProgressIndicator value={progressPct} max={100} />
					</div>
				{:else}
					<div class="flex items-center gap-2">
						...
						<span class="text-sm font-semibold" aria-live="polite"
							>{formatFinal(progressValue)}</span
						>
						<ProgressIndicator value={progressPct} max={100} />
					</div>
				{/if}
					...
					{#if mode === 'self' && canEdit && !selfReadMode}
						...
						{#if authorLine(closure?.selfAssessmentAuthor, closure?.selfAssessmentCreatedAt, closure?.selfAssessment)}
							<p class="text-[11px] text-base-content/40 mt-1">
								{authorLine(
									closure?.selfAssessmentAuthor,
									closure?.selfAssessmentCreatedAt,
			...
			<div class="mb-2">
				...
				{#if mode === 'rh' && !rhReadMode}
					...
					{#if authorLine(closure?.rhAssessmentAuthor, closure?.rhAssessmentCreatedAt, closure?.rhAssessment)}
						<p class="text-[11px] text-base-content/40 mt-1">
							{authorLine(
								closure?.rhAssessmentAuthor,
								closure?.rhAssessmentCreatedAt,
								closure?.rhAssessment,
							)}
						</p>
					{/if}
					<div class="mt-2 flex gap-2">
			...
			<div class="mb-2">
				{#if mode === 'manager' && canEdit && !managerReadMode}
					...
					{#if authorLine(closure?.managerCommentAuthor, closure?.managerCommentCreatedAt, closure?.managerComment)}
						<p class="text-[11px] text-base-content/40 mt-1">
							{authorLine(
								closure?.managerCommentAuthor,
								closure?.managerCommentCreatedAt,
								closure?.managerComment,
							)}
						</p>
					{/if}
					<div class="mt-2 flex gap-2">
