<script lang="ts">
	...
	function tooltipByLevel(competencyId: string, pillarId: string): Record<number, string> {
		return {
	...
	function handleSend(competencyId: string) {
		...
		if (mode === 'manager') {
			...
			const managerComment = draftComments[competencyId] ?? (rating?.managerComment ?? '');
			onRhRate?.(competencyId, level, rating?.rhComment, evaluationId, phase, managerComment);
		} else {
			const fallbackComment = mode === 'rh' ? (rating?.rhComment ?? '') : (rating?.selfComment ?? '');
	...
	<div class="card-body px-0">
		...
		<div class="flex flex-col gap-5">
			{#each competencies as competency (competency.id)}
				...
				{@const ownSavedComment = mode === 'self' ? rating?.selfComment : mode === 'rh' ? rating?.rhComment : rating?.managerComment}
				{@const ownHasComment = hasRealComment(ownSavedComment)}
				{@const isEditingOwn = editing[competency.id] === mode}
				{@const ownHasRating = mode === 'self' ? rating?.selfRating != null : rating?.rhRating != null}
				...
				{@const ownLabel = mode === 'self' ? 'Autoevaluación' : mode === 'rh' ? 'Evaluación de RH' : 'Evaluación de Jefe'}
				{@const ownPlaceholder = mode === 'self' ? 'Escribe tu autoevaluación...' : mode === 'rh' ? 'Escribe evaluación de RH...' : 'Escribe evaluación de Jefe...'}
				{@const ownEmpty = mode === 'self' ? 'Sin autoevaluación' : mode === 'rh' ? 'Sin evaluación de RH' : 'Sin evaluación de Jefe'}
				{@const otherManagerAuthor = rating?.managerCommentAuthor ?? rating?.authorName}
				{@const otherManagerDate = rating?.managerCommentCreatedAt ?? rating?.commentCreatedAt}
				{@const ownAuthor = mode === 'manager' ? (rating?.managerCommentAuthor ?? rating?.authorName) : rating?.authorName}
				{@const ownDate = mode === 'manager' ? (rating?.managerCommentCreatedAt ?? rating?.commentCreatedAt) : rating?.commentCreatedAt}
				<div class="pt-3 first:pt-0">
					...
					<div class="shrink-0">
						{#if mode === 'self'}
							<ScaleRatingSelector
								value={draftLevels[competency.id] ?? (ownReadMode ? rating?.selfRating : undefined)}
								{acceptanceLevel}
								disabled={selfDisabled}
								{levelDefinitions}
								tooltips={tooltipByLevel(competency.id, competency.pillarId ?? pillar.id)}
								readOnlySingle={ownReadMode}
								required={!ownReadMode && canEditOwn}
								invalid={!ownReadMode && canEditOwn && !canSend(competency.id)}
								attempted={!!attempted[competency.id]}
								onChange={(level) => handleRatingChange(competency.id, level)}
							/>
						{:else}
							<ScaleRatingSelector
								value={draftLevels[competency.id] ?? (ownReadMode ? rating?.rhRating : undefined)}
								{acceptanceLevel}
								disabled={rhDisabled}
								{levelDefinitions}
								tooltips={tooltipByLevel(competency.id, competency.pillarId ?? pillar.id)}
					...
					{:else if mode === 'manager' && !rating?.selfRating}
						<span class="badge badge-ghost badge-outline badge-sm shrink-0" title="Autoevaluación">Sin autoevaluación</span>
					{/if}
					</div>

					<!-- Otros comentarios (lectura): todos visibles, solo el propio es editable -->
					{#if mode !== 'self' && hasRealComment(rating?.selfComment)}
						<p class="text-xs text-base-content/50 mt-2"><span class="font-semibold">Autoevaluación:</span> <span class="italic">"{rating?.selfComment}"</span></p>
						{#if authorLine(rating?.authorName, rating?.commentCreatedAt, rating?.selfComment)}
							<p class="text-[11px] text-base-content/40 mt-1">{authorLine(rating?.authorName, rating?.commentCreatedAt, rating?.selfComment)}</p>
						{/if}
					{/if}
					{#if mode !== 'rh' && hasRealComment(rating?.rhComment)}
						<p class="text-xs text-base-content/50 mt-2"><span class="font-semibold">Evaluación de RH:</span> <span class="italic">"{rating?.rhComment}"</span></p>
						{#if authorLine(rating?.authorName, rating?.commentCreatedAt, rating?.rhComment)}
							<p class="text-[11px] text-base-content/40 mt-1">{authorLine(rating?.authorName, rating?.commentCreatedAt, rating?.rhComment)}</p>
						{/if}
					{/if}
					{#if mode !== 'manager' && hasRealComment(rating?.managerComment)}
						<p class="text-xs text-base-content/50 mt-2"><span class="font-semibold">Evaluación de Jefe:</span> <span class="italic">"{rating?.managerComment}"</span></p>
						{#if authorLine(otherManagerAuthor, otherManagerDate, rating?.managerComment)}
							<p class="text-[11px] text-base-content/40 mt-1">{authorLine(otherManagerAuthor, otherManagerDate, rating?.managerComment)}</p>
						{/if}
					{/if}

					<!-- Comentario propio: lectura con pencil, edición exclusiva -->
					{#if showCommentInput && canEditOwn && !ownReadMode}
						<label class="label px-0 py-1 mt-2" for={`comment-${competency.id}`}>
						...
						<textarea
							...
							oninput={(e) => {
								handleCommentInput(competency.id, (e.target as HTMLTextAreaElement).value);
						...
						{#if authorLine(ownAuthor, ownDate, ownSavedComment)}
							<p class="text-[11px] text-base-content/40 mt-1">{authorLine(ownAuthor, ownDate, ownSavedComment)}</p>
						{/if}
					...
					{:else if showCommentInput || ownHasComment}
						<div class="mt-2">
							...
							<p class="text-sm text-base-content/70 bg-base-200 rounded p-2">
								{ownHasComment ? ownSavedComment : ownHasRating ? 'Sin comentario' : ownEmpty}
