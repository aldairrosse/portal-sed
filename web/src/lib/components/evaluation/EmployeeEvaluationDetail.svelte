<script lang="ts">
	...
	import {
		...
		isInicioAnio as isInicioAnioPhase,
	...
	function getAcceptanceLevel(competencyId: string): number | undefined {
		return getCompetencyAcceptanceLevel(
			competencyId,
			assignment?.profileId ?? 'colaborador',
		)?.level;
	}

	function buildAcceptanceLevels(
	...
	const headingLabel = $derived(
		isMedioAnio
			? 'Evaluación de avance de medio año'
	...
	async function handleSelfRate(
		...
		try {
			...
			notifications.errorWithCode(
				message || 'Error al guardar autoevaluación',
				code,
			);
		}
	}

	async function handleRhRate(
		competencyId: string,
		level: 1 | 2 | 3 | 4 | 5,
		comment?: string,
		_evaluationId?: string,
		_phase?: 'avance' | 'cierre',
		managerComment?: string,
	) {
		try {
			await rhRateCompetency(
				employeeId,
				competencyId,
				level,
				comment,
			...
			const { message, code } = errorWithCode(e);
			notifications.errorWithCode(
				message || 'Error al guardar evaluación RH',
				code,
			);
		}
	}
				...
				<section class="tab-content bg-base-100 border-base-300 p-6">
					...
					{#if tab === 'metas'}
						...
						{#if isInicioAnio && viewerMode === 'self' && !hasPersonalGoals}
							<div class="flex flex-col gap-3">
								<p class="text-sm text-base-content/30 italic">
									No has registrado metas para este ciclo.
								</p>
								<a
									href="/objetivos/asignacion"
									class="btn btn-primary btn-sm w-fit">Ir a asignación</a
								>
							</div>
						...
						{:else if showEmptyMedioFin}
							<p class="text-sm text-base-content/30 italic">
								No se registraron metas para evaluación.
							</p>
						{:else if categories.length === 0}
							<p class="text-sm text-base-content/30 italic">
								No hay categorías de metas configuradas.
							</p>
						{:else}
							<div class="flex flex-col gap-6">
									...
									{#if goals.length > 0}
										<div class="card bg-base-100 border border-base-300">
											<div class="card-body px-0">
												<h4
													class="text-base font-semibold text-base-content mb-3"
												>
													{category.name}
												</h4>
					...
					{#if tab === 'competencias'}
						<div class="flex items-center justify-between mb-4">
							...
							<a
								href="/evaluacion/9x9/competencias/{employeeId}"
