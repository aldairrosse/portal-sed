...
<div class="flex items-center gap-2">
	{#if singleLevel == null}
		<span
			class="text-xs shrink-0 {showError
				? 'text-error'
				: 'text-base-content/50'}"
			role={showError ? 'alert' : undefined}>Selecciona 1-5</span
		>
	{/if}
	<div
		class="flex gap-4"
	...
	>
		{#if singleLevel != null}
			...
			<div
				class="flex flex-col items-center gap-0.5 {singleTip
					? 'tooltip tooltip-top tooltip-end'
					: ''}"
			>
				{#if singleTip}<div
						class="tooltip-content max-w-[280px] whitespace-pre-wrap h-auto text-left text-wrap p-2 text-xs"
					>
						{singleTip}
					</div>{/if}
				<button
		...
		{:else}
			{#each levels as level (level)}
				...
				<div
					class="flex flex-col items-center gap-0.5 {tip
						? 'tooltip tooltip-top tooltip-end'
						: ''}"
				>
					{#if tip}<div
