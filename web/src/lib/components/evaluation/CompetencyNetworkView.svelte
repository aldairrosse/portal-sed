<script lang="ts">
	...
	const profileId = $derived(
		(employeeNode?.profileId ?? 'colaborador') as EvaluationProfile,
	);
	const displayName = $derived(
		employeeName || employeeNode?.name || 'Empleado',
	);

	const pillars = $derived(getPillars());
	const levelDefinitions = $derived(getLevelDefinitions());
	const allAcceptance = $derived(
		getCompetencyAcceptanceLevelsByProfile(profileId),
	);
	...
	const ratings = $derived<CompetencyRating[]>(
		getCompetencyRatings(employeeId),
	);
...
<div class="flex flex-col gap-6">
			...
			{#if ratings.length === 0}
				<div class="text-center py-8">
					<p class="text-sm text-base-content/40">
						No hay evaluaciones registradas para {displayName}.
					</p>
				</div>
			{/if}
		</div>
	{:else}
		<div id="panel-radar" role="tabpanel" aria-labelledby="view-radar">
			<RadarChart {pillarGroups} employeeName={displayName} {isSelf} />
		</div>
	{/if}
</div>
