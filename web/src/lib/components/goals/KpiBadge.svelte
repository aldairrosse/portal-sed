<script lang="ts">
	...
	let pct = $derived.by(() => {
		...
		if (kpi.direction === 'ascendente') {
			return Math.min(
				Math.max((kpi.currentValue! / kpi.targetValue!) * 100, 0),
				100,
			);
		}
		// descendente: inverse ratio (closer to zero = better)
		return Math.min(
			Math.max((1 - kpi.currentValue! / kpi.targetValue!) * 100, 0),
			100,
		);
	});
...
<span
	class="badge badge-outline badge-sm gap-1 whitespace-nowrap max-w-[16rem]"
...
>
	...
	{#if hasData}
		...
		{#if delta && delta.type !== 'neutral'}
			<span
				class="badge badge-xs {delta.type === 'positive'
					? 'badge-success'
					: 'badge-error'}"
			>
