<script lang="ts">
	...
	import {
		createGlobalGoal,
		updateGlobalGoal,
		type CreateGlobalGoalRequest,
		type CreateAssignmentRequest,
		type CreateRuleRequest,
	} from '$lib/api/globalGoals';
	...
	const RULE_TYPE_LABELS: Record<
		'department' | 'min_direct_reports' | 'role',
		string
	> = {
	...
	$effect(() => {
		...
		initializedFor = key;
		{
			...
			rules = (init?.rules ?? []).map((r) => ({
				ruleType:
					r.rule_type === 'min_direct_reports'
						? 'min_direct_reports'
						: r.rule_type === 'role'
							? 'role'
							: 'department',
	...
	async function resolveEmployeeName(employeeId: string): Promise<string> {
		...
		const data = (
			res.data as { data?: { firstName?: string; lastName?: string } }
		)?.data;
	...
	function addDepartmentRule() {
		rules = [
			...rules,
			{
				...
				minDirectReports: 0,
				profileId: '',
				defaultWeight: weight > 0 ? weight : 0,
	...
	function validate(): string {
		...
		if (targetValue <= 0 && direction !== 'descendente' && unit !== 'binario')
			return 'El valor objetivo debe ser mayor a 0';
		if (direction === 'descendente') {
			if (baselineValue === null || baselineValue <= targetValue)
				return 'Para objetivos descendentes, el valor inicial debe ser mayor al objetivo';
		}
		if (!goalId && assignments.length === 0 && rules.length === 0)
			return 'Selecciona al menos un empleado o una regla';
		for (const a of assignments) {
			if (!(a.weight >= 0 && a.weight <= 100))
				return 'Ponderación de empleado inválida (0-100)';
			if (a.targetValue < 0)
				return 'El valor objetivo del empleado debe ser mayor o igual a 0';
		}
		for (const r of rules) {
			if (!(r.defaultWeight >= 0 && r.defaultWeight <= 100))
				return 'Ponderación por defecto inválida (0-100)';
			if (r.defaultTarget < 0)
				return 'El objetivo por defecto debe ser mayor o igual a 0';
			if (r.ruleType === 'department' && !r.departmentId)
				return 'Selecciona un departamento';
			if (r.ruleType === 'role' && !r.profileId) return 'Selecciona un perfil';
			if (r.ruleType === 'min_direct_reports' && !(r.minDirectReports > 0))
				return 'El mínimo de reportes directos debe ser mayor a 0';
		}
		if (rules.filter((r) => r.ruleType === 'min_direct_reports').length > 1)
			return 'Solo puede haber una regla de mínimo de reportes directos';
		return '';
	}
					...
					? rules.map((r) => ({
							...
							...(r.ruleType === 'department'
								...
								: r.ruleType === 'role'
									...
									: { min_direct_reports: r.minDirectReports }),
	...
	<div class="modal-box max-w-2xl max-h-[90vh] overflow-y-auto">
		<div class="flex items-center justify-between mb-4">
			<h3 class="font-bold text-lg">
				{goalId ? 'Editar' : 'Nueva'} meta {goalKind === 'qualitative'
					? 'cualitativa'
					: 'cuantitativa'}
			</h3>
			<button
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
			<div class="form-control sm:col-span-2">
				<label class="label" for="global-goal-name"
					><span class="label-text text-xs">Nombre de la meta</span></label
				>
				<input
					id="global-goal-name"
					type="text"
			...
			<div class="form-control sm:col-span-2">
				<label class="label" for="global-goal-desc"
					><span class="label-text text-xs">Descripción</span></label
				>
				<textarea
					id="global-goal-desc"
			...
			<div class="form-control">
				<span class="label"
					><span class="label-text text-xs">Unidad de medida</span></span
				>
				<CustomSelect
			...
			<div class="form-control">
				<p class="label">
					<span class="label-text text-xs">Dirección del objetivo</span>
				</p>
			...
			<div class="form-control">
				<label class="label" for="global-goal-weight"
					><span class="label-text text-xs">Ponderación (%)</span></label
				>
				<input
					id="global-goal-weight"
					type="number"
					class="input input-bordered input-sm w-full"
					bind:value={weight}
					min={0}
			...
			<div class="form-control">
				<label class="label" for="global-goal-target"
					><span class="label-text text-xs">Valor objetivo a alcanzar</span
					></label
				>
				<input
					id="global-goal-target"
					type="number"
					class="input input-bordered input-sm w-full"
			...
			{#if direction === 'descendente'}
				<div class="form-control">
					<label class="label" for="global-goal-baseline"
						><span class="label-text text-xs">Valor inicial</span></label
					>
					<input
						id="global-goal-baseline"
						type="number"
						class="input input-bordered input-sm w-full"
						bind:value={baselineValue}
						min={0}
						step={0.01}
						required
		...
		<div class="border border-base-300 rounded-lg p-3 mb-3">
			...
			<div
				class="grid grid-cols-[minmax(0,1fr)_6rem_7rem_2rem] gap-2 mb-1 opacity-60"
			>
				...
				{:else}
					<div class="space-y-2 max-h-64 overflow-y-auto overflow-x-hidden">
						{#each assignments as a, i (a.employeeId)}
							<div
								class="grid grid-cols-[minmax(0,1fr)_6rem_7rem_2rem] gap-2 items-center"
							>
								...
								<div class="form-control">
									<input
										...
										max={100}
										step={0.1}
										aria-label={`Ponderación de ${a.employeeName}`}
										placeholder="Peso %"
									/>
								</div>
								<div class="form-control">
									<input
										type="number"
										class="input input-bordered input-sm w-full"
										bind:value={a.targetValue}
										min={0}
										step={0.01}
										aria-label={`Valor objetivo de ${a.employeeName}`}
		...
		<div class="border border-base-300 rounded-lg p-3 mb-3">
			<div class="flex items-center justify-between mb-2">
				<span class="label-text text-xs font-semibold"
					>Reglas de asignación (opcional)</span
				>
				<div class="flex items-center gap-2">
					<button
			...
			{#if rules.length > 0}
				<div
					class="grid grid-cols-[6rem_minmax(0,1fr)_6rem_7rem_2rem] gap-2 mb-1 opacity-60"
				>
			...
			{#if rules.length > 0}
				<div class="space-y-2">
					{#each rules as r, i (i)}
						<div
							class="grid grid-cols-[6rem_minmax(0,1fr)_6rem_7rem_2rem] gap-2 items-center"
						>
							<span class="badge badge-sm badge-outline justify-self-start"
								>{RULE_TYPE_LABELS[r.ruleType]}</span
							>
							...
							{:else}
								<div class="form-control">
									<input
										type="number"
										class="input input-bordered input-sm w-full"
										bind:value={r.minDirectReports}
										min={1}
										step={1}
										aria-label="Mínimo de reportes directos"
										placeholder="Min. reportes"
									/>
								</div>
							{/if}
							<div class="form-control">
								<input
									type="number"
									class="input input-bordered input-sm w-full"
									bind:value={r.defaultWeight}
									min={0}
									max={100}
									step={0.1}
									aria-label="Ponderación por defecto"
									placeholder="Peso %"
								/>
							</div>
							<div class="form-control">
								<input
									type="number"
									class="input input-bordered input-sm w-full"
									bind:value={r.defaultTarget}
									min={0}
									step={0.01}
									aria-label="Objetivo por defecto"
									placeholder="Objetivo"
								/>
							</div>
							<button
