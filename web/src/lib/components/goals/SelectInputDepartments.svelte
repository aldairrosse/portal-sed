...
<div class="w-full {maxWidthClass}">
	...
	<div class="dropdown dropdown-top w-full {maxWidthClass}">
		...
		{#if open}
			<button type="button" class="fixed inset-0 z-10 bg-transparent border-0 p-0 cursor-default" aria-label="Cerrar menú" onclick={() => (open = false)} onkeydown={(e) => { if (e.key === 'Escape' || e.key === 'Enter' || e.key === ' ') open = false; }}></button>
			<div
