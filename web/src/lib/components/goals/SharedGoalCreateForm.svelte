<script lang="ts">
	...
	import {
		...
		getTeamMembers,
		type TeamMember,
	} from '$lib/stores/teamStore.svelte';
	import {
		createSharedGoal,
		updateSharedGoal,
		type CreateSharedGoalRequest,
		type CreateMemberRequest,
		type UpdateSharedGoalRequest,
	...
	let memberForm = $state<
		Record<
			string,
			{ employee_id: string; weight: number; target_value: number }
		>
	>({});
			...
			if (initial.members && initial.members.length > 0) {
				...
				for (const m of initial.members) {
					next[m.employee_id] = {
						...
						weight: m.weight,
						target_value: m.target_value || initial.target_value || 0,
					};
				}
				memberForm = next;
	...
	function setMemberWeight(memberId: string, value: number) {
		memberForm = {
			...memberForm,
			[memberId]: { ...memberForm[memberId], weight: value || weight || 1 },
		};
	}
	...
	function setMemberTarget(memberId: string, value: number) {
		memberForm = {
			...memberForm,
			[memberId]: { ...memberForm[memberId], target_value: value || 0 },
		};
	}
	...
	function validate(): string {
		...
		if (Object.keys(memberForm).length === 0)
			return 'Selecciona al menos un miembro del grupo';
		for (const m of Object.values(memberForm)) {
			if (m.weight < 0.01 || m.weight > 100)
				return 'Peso debe ser >0 y ≤100 para cada miembro';
			if (
				...
				return 'El valor objetivo del miembro debe ser mayor a 0';
		}
		return '';
	}
	...
	async function handleSave() {
		...
		try {
			if (goalId) {
				const membersPayload: CreateMemberRequest[] = Object.values(
					memberForm,
				).map((m) => ({
					employee_id: m.employee_id,
					weight: m.weight,
			...
			error =
				e instanceof Error ? e.message : 'Error al guardar la meta compartida';
		} finally {
			saving = false;
		}
	}
	...
	<div class="modal-box max-w-2xl max-h-[90vh] overflow-y-auto">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
			<div class="form-control">
				<label class="label" for="shared-name"
			...
			<div class="form-control">
				<label class="label" for="shared-desc"
			...
			<div class="form-control">
				<label class="label" for="shared-unit"
					><span class="label-text text-xs">Unidad de medida</span></label
				>
				<CustomSelect
			...
			<div class="form-control">
				<label class="label" for="shared-weight"
			...
			<div class="form-control">
				<label class="label" for="shared-target"
					><span class="label-text text-xs">Valor objetivo</span></label
				>
				<input
					id="shared-target"
					type="number"
			...
			{#if direction === 'descendente'}
				<div class="form-control">
					<label class="label" for="shared-goal-baseline"
						><span class="label-text text-xs">Valor inicial</span></label
					>
					<input
						id="shared-goal-baseline"
						type="number"
						class="input input-bordered input-sm w-full"
		...
		<div class="form-control">
			<span class="label"
				><span class="label-text text-xs">Miembros del grupo</span></span
			>
			{#if members.length > 0}
				<div
					class="grid grid-cols-[1.25rem_minmax(0,1fr)_6rem_7rem] gap-2 mb-1 opacity-60"
				>
					<span></span>
			...
			{:else}
				<div class="space-y-2 max-h-64 overflow-y-auto pr-1 overflow-x-hidden">
					{#each members as member (member.id)}
						<div
							class="grid grid-cols-[1.25rem_minmax(0,1fr)_6rem_7rem] gap-2 items-center"
						>
							<input
								type="checkbox"
							...
							{#if memberForm[member.id]}
								<input
									...
									oninput={(e) =>
										setMemberWeight(member.id, Number(e.currentTarget.value))}
								/>
								<input
									type="number"
									class="input input-bordered input-sm w-full"
									...
									oninput={(e) =>
										setMemberTarget(member.id, Number(e.currentTarget.value))}
								/>
							{/if}
						</div>
					{/each}
