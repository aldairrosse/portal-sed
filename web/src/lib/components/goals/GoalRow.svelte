...
<tr>
	...
	{:else}
		...
		<td class="text-sm text-base-content/70"
			>{goal.weight}%{#if (goal as unknown as { source?: string }).source === 'global' || (goal as unknown as { source?: string }).source === 'shared'}<span
					class="badge badge-ghost badge-sm font-mono ml-1 align-middle"
					title="Peso ponderado"
					>{effective.toFixed(2).replace(/\.?0+$/, '')}%</span
				>{/if}</td
		>
		<td>
			{#if phase === 'fin-anio'}
