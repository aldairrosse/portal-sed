<script lang="ts">
    ...
    async function handleSaveEdit() {
        const err = validateGoal({
            ...
            baselineValue:
                editDirection === "descendente" ? editBaseline : undefined,
    ...
    function handleToggleEditKpi(kpiId: string) {
        editKpiIds = editKpiIds.includes(kpiId)
            ? editKpiIds.filter((id) => id !== kpiId)
            : [...editKpiIds, kpiId];
    }
...
<tr>
    {#if isEditing}
        <td colspan={phase !== "fin-anio" ? 5 : 4} class="p-0">
            <div
                class="border border-base-300 rounded-lg p-4 m-2 bg-base-200/50"
            >
                {#if editError}<div
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                    <div class="form-control">
                        <label class="label" for="edit-goal-name-{goal.id}"
                    ...
                    <div class="form-control">
                        <label class="label" for="edit-goal-desc-{goal.id}"
                            ><span class="label-text text-xs">Descripción</span
                            ></label
                        >
                        <textarea
                            id="edit-goal-desc-{goal.id}"
                    ...
                    <div class="form-control">
                        <label class="label" for="edit-goal-unit-{goal.id}"
                        ...
                        <div class="flex gap-4 pt-1">
                            ...
                            >
                                <input
                                    ...
                                    name="edit-goal-dir-{goal.id}"
                                    value="ascendente"
                                    checked={editDirection === "ascendente"}
                                    onchange={() => {
                                        editDirection = "ascendente";
                                        if (editBaseline !== undefined)
                            ...
                            >
                                <input
                                    ...
                                    name="edit-goal-dir-{goal.id}"
                                    value="descendente"
                                    checked={editDirection === "descendente"}
                                    onchange={() =>
                                        (editDirection = "descendente")}
                                />
                    ...
                    <div class="form-control">
                        <label class="label" for="edit-goal-weight-{goal.id}"
                            ><span class="label-text text-xs">Peso (%)</span
                            ></label
                        >
                        <input
                            id="edit-goal-weight-{goal.id}"
                            type="number"
                            class="input input-bordered input-sm w-full"
                    ...
                    <div class="form-control">
                        <label class="label" for="edit-goal-target-{goal.id}"
                    ...
                    {#if editDirection === "descendente"}
                        <div class="form-control">
                            <label
                                class="label"
                                for="edit-goal-baseline-{goal.id}"
                                ><span class="label-text text-xs"
                                    >Punto de partida</span
                                ></label
                            >
                            <input
                                id="edit-goal-baseline-{goal.id}"
                                type="number"
                                class="input input-bordered input-sm w-full"
                                bind:value={editBaseline}
                                min={0}
                                step={0.01}
                                required
                            />
                ...
                {#if allKpis && allKpis.length > 0}
                    <div class="form-control mb-3">
                        <label class="label" for="edit-goal-kpi-{goal.id}"
                            ><span class="label-text text-xs"
                                >Indicadores clave (KPI)</span
                            ></label
                        >
                        <div
                            id="edit-goal-kpi-{goal.id}"
                            class="flex flex-wrap gap-2"
                        >
                            {#each allKpis as kpi (kpi.id)}
                                <label
                                    class="flex items-center gap-1.5 cursor-pointer px-2 py-1 rounded border border-base-300 hover:bg-base-200/50 text-xs"
                                >
                                    <input
                                        type="checkbox"
                                        class="checkbox checkbox-xs checkbox-primary"
                                        checked={editKpiIds.includes(kpi.id)}
                                        onchange={() =>
                                            handleToggleEditKpi(kpi.id)}
                                    />
                                    <span>{kpi.name}</span>
                                </label>
                            {/each}
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <button
                        ...
                        ><Check class="w-4 h-4" /> Guardar meta</button
                    >
                </div>
            </div>
        </td>
            ...
            <td class="text-right">
                <div class="flex items-center justify-end gap-1">
                    ...
                    {:else if onRequestChange}
                        <button
                            class="btn btn-ghost btn-xs text-warning relative"
