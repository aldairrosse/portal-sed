...
<div>
	...
	<div class="mb-6">
		...
		<p class="text-sm text-base-content/60">
			Niveles de aceptación para el perfil. Asigne un nivel de aceptación a cada
			competencia.
		</p>
	</div>
					...
					{#each pillarComps as competency (competency.id)}
						...
						>
							<div class="flex-1 min-w-0">
								<span class="text-sm font-medium block">{competency.name}</span>
							...
							<CustomSelect
								...
								onChange={(v) => handleLevelChange(competency.id, v)}
	...
	{#if competencies.length === 0}
		<div class="text-center py-12 text-base-content/50 text-sm">
			No hay competencias registradas. Agregue competencias para asignar niveles
			de aceptación.
		</div>
	{/if}
</div>
