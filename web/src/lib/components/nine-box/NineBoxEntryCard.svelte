...
>
	<div class="modal-box max-w-md max-h-[80vh] overflow-hidden flex flex-col">
		...
		<div class="flex items-start justify-between gap-2 mb-4">
			<div>
				...
				<p class="text-sm text-base-content/50 mt-1">
					{entries.length}
					{entries.length === 1 ? 'empleado' : 'empleados'}
				</p>
			</div>
			<form method="dialog">
				<button
		...
		<div class="flex gap-4 mb-4 p-3 bg-base-200 rounded-lg">
			<div class="flex-1 text-center">
				...
				<span class="text-xl font-bold text-base-content"
					>{TIER_LABELS[perfTier]}</span
				>
				<span class="text-xs text-base-content/40 block mt-0.5"
					>Tier {perfTier}</span
				>
			</div>
		...
		<!-- Employee list -->
		<ul
			class="divide-y divide-base-200 overflow-y-auto overflow-x-hidden h-full"
		>
			{#if entries.length === 0}
		...
		<p class="text-xs text-base-content/30 text-center mt-4">
			Los tiers se calculan automáticamente desde avance de metas y evaluación
			de competencias.
		</p>
	</div>
