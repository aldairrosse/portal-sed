<script lang="ts">
	...
	interface Props {
		...
		isRH: boolean;
		onOpenConfig: (quadrant: number) => void;
		onCellClick: (
			entries: NineBoxEntry[],
	...
	let { entries, quadrantDefs, isRH, onCellClick, onOpenConfig }: Props =
		$props();
	...
	let activeDescendantId = $derived(
		`nb-cell-${getQuadrantNumber(activePerf, activePot)}`,
	);
			...
			case ' ':
				e.preventDefault();
				{
					...
					if (cellEntries.length > 0)
						onCellClick(cellEntries, activePerf, activePot);
				}
				return;
			default:
				return;
	...
	function handleOpenConfig(e: Event, quadrant: number) {
		e.preventDefault();
		e.stopPropagation();
		onOpenConfig(quadrant);
	}
</script>
...
{:else}
	...
	<div
		...
		onfocus={() =>
			announce(
				`Matriz 3x3 cargada. Use flechas para navegar. Celda activa: ${PERF_LABELS[activePerf]} desempeño, ${POT_LABELS[activePot]} potencial.`,
			)}
	>
		...
		{#each potTiers as pot (pot)}
			...
			{#each perfTiers as perf (perf)}
				...
				<button
					...
					aria-label="Cuadrante {quadrant}: {POT_LABELS[
						pot
					]} potencial, {PERF_LABELS[perf]} desempeño, {count} empleados"
					class="relative flex flex-col items-center justify-center gap-1 rounded-lg transition-all duration-100 cursor-pointer hover:brightness-95 min-h-[6rem] p-2 {isActive
						? 'ring-2 ring-primary ring-offset-2'
						: ''} {pressedCell === quadrant ? 'scale-[0.98]' : ''}"
					style="background-color: {qDef?.colorHex ?? '#6B7280'};"
					onmousedown={(e) => {
						if (e.target === e.currentTarget) {
							pressedCell = quadrant;
						}
					}}
					onmouseup={() => {
						pressedCell = null;
					...
					onfocus={() => {
						...
						activePot = pot;
						announce(
							`Celda: ${PERF_LABELS[perf]} desempeño, ${POT_LABELS[pot]} potencial. ${count} empleados.`,
						);
					}}
				>
					{#if isRH}
						<a
							class="btn btn-xs btn-ghost gap-1 absolute top-1 right-1 rounded-full px-2 py-1 text-[10px] font-semibold bg-base/20 hover:bg-base/30"
							style="border-left: 3px solid {qDef?.colorHex};"
							href="#top"
							onclick={(e) => handleOpenConfig(e, quadrant)}
							aria-haspopup="dialog"
							aria-label="Configurar cuadrante {quadrant}"
						>
		...
		<div
			role="columnheader"
			class="text-center text-[11px] font-medium text-base-content/50 flex items-center justify-center"
		>
			Bajo
		</div>
		<div
		...
		<div
			role="presentation"
			class="text-center text-[10px] font-semibold text-base-content/40 pt-1 flex items-center justify-center"
		>
			Desempeño
		</div>
		<div role="presentation" class="min-h-[2.5rem]"></div>
