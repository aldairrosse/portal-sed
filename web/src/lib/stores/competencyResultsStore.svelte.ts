...
export function getItems(): CompetencyResultItem[] {
	return items;
}
...
export function getScope(): 'all' | 'team' {
	return scopeFilter;
}
	...
	try {
		...
		if (res.error) {
			throw new Error(
				apiErrorMessage(
					res.error,
					'Error al cargar resultados de competencias',
				),
			);
		}
		...
		error =
			e instanceof Error
				? e.message
				: 'Error desconocido al cargar resultados de competencias';
		items = [];
	} finally {
		loading = false;
	}
