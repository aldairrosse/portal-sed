import type {
	...
	GoalClosure,
	EvaluationStatus,
} from '$lib/types/evaluation-result';
import {
	getActivePhase,
	getActiveCycleId,
	loadCycle,
 ...
 */
function upsertQuery(employeeId: string): {
	employee_id: string;
	cycle_id?: string;
} {
...
async function ensureCycleLoaded(): Promise<string> {
	...
	if (!cycleId)
		throw new Error('Sin ciclo activo: no se puede resolver evaluationId');
	return cycleId;
}
...
async function findEvaluationInList(
	...
	do {
		const { data: listData, error: listError } = await client.GET(
			'/evaluations',
			{
...
async function ensureEvaluationId(
	employeeId: string,
): Promise<string | undefined> {
...
async function refreshEvaluationId(
	employeeId: string,
): Promise<string | undefined> {
...
function normalizeApiData(
	...
	const competencyRatings: CompetencyRating[] = (
		...
		selfRating:
			c.selfRating != null ? (c.selfRating as 1 | 2 | 3 | 4 | 5) : undefined,
		selfComment: c.selfComment ?? '',
		rhRating:
			c.rhRating != null ? (c.rhRating as 1 | 2 | 3 | 4 | 5) : undefined,
	...
	const goalClosures: GoalClosure[] = (detail?.goals ?? []).map((g, i) => ({
		...
		selfAssessmentCreatedAt:
			g.createdAt ?? g.updatedAt ?? g.managerCommentCreatedAt ?? undefined,
		rhAssessmentAuthor: g.authorName ?? g.managerCommentAuthor ?? undefined,
		rhAssessmentCreatedAt:
			g.updatedAt ?? g.createdAt ?? g.managerCommentCreatedAt ?? undefined,
		managerCommentAuthor: g.managerCommentAuthor ?? g.authorName ?? undefined,
		managerCommentCreatedAt:
			g.managerCommentCreatedAt ?? g.updatedAt ?? g.createdAt ?? undefined,
	}));
...
export function errorWithCode(e: unknown): {
	message: string;
	code: string | null;
} {
	if (e instanceof HttpNotFoundError) {
		return {
			message: 'Evaluación no encontrada, se creará automáticamente',
...
export async function load(
	...
	if (loadPromise) {
		if (employeeId === lastEmployeeId && viewerMode === lastViewerMode)
			return loadPromise;
		try {
			await loadPromise;
		} catch {
				...
				if (apiError) {
					throw new Error(
						typeof apiError === 'string'
							? apiError
							: 'Error al cargar competencias del empleado',
					);
				}

				// Map ratings to competencies for normalizeApiData
				const resp = apiData as
					| {
							employeeId?: string;
							cycleId?: string;
							ratings?: Array<{
			...
			try {
				...
				if (!evalId) {
					...
					data = normalizeApiData(
						apiData as Parameters<typeof normalizeApiData>[0],
						empId,
					);
...
export function getCompetencyRatings(employeeId: string): CompetencyRating[] {
	return (data?.competencyRatings ?? []).filter(
...
export function getGoalClosures(employeeId: string): GoalClosure[] {
	return (data?.goalClosures ?? []).filter(
		(gc) => gc.employeeId === employeeId,
	);
}

export function getGoalClosure(
	employeeId: string,
	goalId: string,
): GoalClosure | undefined {
...
export function getEvaluationStatus(
	...
	const ratedIds = new SvelteSet(
		empRatings
			.filter((cr) => cr.selfRating != null)
			.map((cr) => cr.competencyId),
	);
...
export async function rateCompetency(
	...
	const competencies = allRatings
		.map((r) => ({
			competencyId: r.competencyId,
			rating: r.competencyId === competencyId ? level : r.selfRating,
			comments: r.competencyId === competencyId ? comment : r.selfComment,
		}))
		.filter(
	...
	try {
		...
		if (apiError) {
			...
			if (isNotFoundCode(code, apiError)) {
				...
				const { data: retryData, error: retryError } = await client.PUT(
					'/evaluations/{id}/self-evaluation',
					{
						params: {
							path: { id: resolveEvaluationId(employeeId) },
							header: { 'If-Match': currentVersion() },
						},
						body: { competencies },
					},
		...
		if (e instanceof HttpNotFoundError) {
			...
			const { data: catchRetryData, error: retryError } = await client.PUT(
				'/evaluations/{id}/self-evaluation',
				{
					params: {
						path: { id: resolveEvaluationId(employeeId) },
			...
			if (retryError)
				throw new Error('Error al guardar autoevaluación', { cause: e });
...
export async function closeGoal(
	...
	try {
		const { data: respData, error: apiError } = await client.PUT(
			'/evaluations/{id}/goal-state',
			{
				params: {
					path: { id: pathIdFor(employeeId) },
					query,
					header: { 'If-Match': currentVersion() },
				},
				body,
		...
		let resp: { version?: number | null } | undefined = respData as
			| { version?: number | null }
			| undefined;
		if (apiError) {
			await refreshEvaluationId(employeeId);
...
export async function rhRateCompetency(
	...
	const competencies = allRatings
		.map((r) => ({
			competencyId: r.competencyId,
			rating:
				r.competencyId === competencyId ? level : (r.rhRating ?? r.selfRating),
			comments:
	...
	try {
		...
		if (
			e instanceof Error &&
			(e as Error & { code?: unknown }).code !== undefined
		)
			throw e;
		const err = new Error(message || 'Error al guardar evaluación RH');
		(err as Error & { code?: string | null }).code = code;
...
export async function rhAssessGoal(
	...
	try {
		...
		if (apiError && !isNotFoundCode(errorWithCode(apiError).code, apiError)) {
			...
			if (isVersionConflict(code, message)) await reload();
			const err = new Error(
				message || 'Error al guardar evaluación RH de la meta',
			);
			(err as Error & { code?: string | null }).code = code;
			throw err;
		}
		...
		if (retryError) {
			const { message, code } = errorWithCode(retryError);
			if (isVersionConflict(code, message)) await reload();
			throw new Error(message || 'Error al guardar evaluación RH de la meta', {
				cause: e,
			});
		}
		syncVersion(retryData as { version?: number | null });
...
export async function addManagerComment(
	...
	try {
		...
		if (apiError && !isNotFoundCode(errorWithCode(apiError).code, apiError)) {
			...
			if (isVersionConflict(code, message)) await reload();
			const err = new Error(
				message || 'Error al guardar comentario del manager',
			);
			(err as Error & { code?: string | null }).code = code;
			throw err;
		}
		...
		if (retryError) {
			const { message, code } = errorWithCode(retryError);
			if (isVersionConflict(code, message)) await reload();
			throw new Error(message || 'Error al guardar comentario del manager', {
				cause: e,
			});
		}
		syncVersion(retryData as { version?: number | null });
...
export async function submitRHEvaluation(employeeId: string): Promise<void> {
	...
	if (!evaluationId)
		throw new Error('Sin evaluationId: llama load(employeeId) primero');
