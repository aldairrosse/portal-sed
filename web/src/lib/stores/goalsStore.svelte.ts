...
function normalizeApiData(
	...
	for (const [source, items] of [
		['global', apiAssignment?.global_goals ?? []],
		['shared', apiAssignment?.shared_goals ?? []],
	] as const) {
		for (const raw of items) {
			institutionalGoals.push({
				...
				direction:
					(raw.direction as 'ascendente' | 'descendente') ?? 'ascendente',
	...
	for (const ac of apiCategories) {
		cats.push({
			...
			weight: ac.weight ?? 0,
			effectiveWeight:
				(ac.effective_weight as number | undefined) ??
				((ac as Record<string, unknown>).effective_weight as
					| number
					| undefined),
			pillarId: (ac as Record<string, unknown>)?.pillar_id as
				| string
				| undefined,
		});
		...
		for (const ag of ac.goals ?? []) {
			...
			goals.push({
				...
				direction:
					(ag.direction as 'ascendente' | 'descendente') ?? 'ascendente',
				goalKind:
					ag.goal_kind === 'quantitative'
						? 'quantitative'
				...
				pendingProposal: ag.pending_proposal
					? mapToGoalProposal(ag.pending_proposal)
					: undefined,
			});
				...
				if (kpiRef.id) {
					...
					if (!kpisMap.has(kpiRef.id)) {
						kpisMap.set(kpiRef.id, {
							...
							direction:
								(kpiRef.direction as 'ascendente' | 'descendente') ??
								'ascendente',
	...
	if (apiAssignment?.id) {
		assignments.push({
			...
			status:
				(apiAssignment.status as 'borrador' | 'enviada' | undefined) ??
				'borrador',
		...
		assignments.push({
			...
			createdAt:
				(apiAssignment?.created_at as string) ?? new Date().toISOString(),
			updatedAt:
				(apiAssignment?.created_at as string) ?? new Date().toISOString(),
		});
	}
...
async function _doLoad(empIdOverride?: string): Promise<void> {
	...
	try {
		...
		const apiCategories =
			(catsRes.data as { items?: Array<unknown> })?.items ?? [];
		...
		console.debug('[debug-assigned] store raw', {
			institutional: storeState.data.institutionalGoals.map((g) => ({
				src: g.source,
				w: g.weight,
				ew: g.effectiveWeight,
			})),
			cW: getCycleWeights(),
			tW: getTeamWeights(),
			rawGlobal: (apiAssignment as unknown as { global_goals?: unknown })
				?.global_goals,
			rawShared: (apiAssignment as unknown as { shared_goals?: unknown })
				?.shared_goals,
		});
		...
		storeState.error =
			e instanceof Error
				? e.message
				: 'Error desconocido al cargar datos de objetivos';
	} finally {
				...
				client
					...
					.then(({ data, error }) => {
						if (data && !error) {
							return {
								id: g.id,
								comments: (data as unknown as GoalComment[]).map(
									mapToGoalComment,
								),
							};
						}
...
export function getCategoryProgressAverage(categoryId: string): number {
	const catGoals = (storeState.data?.goals ?? []).filter(
	...
	const cat = (storeState.data?.categories ?? []).find(
		(c) => c.id === categoryId,
	);
...
export function getGoalsByCategory(categoryId: string): Goal[] {
	return (storeState.data?.goals ?? []).filter(
		(g) => g.categoryId === categoryId,
	);
}
...
export function getKpisForGoal(goalId: string): KPI[] {
	const linkKpiIds = (storeState.data?.goalKpiLinks ?? [])
		...
		.map((link) => link.kpiId);
	return (storeState.data?.kpis ?? []).filter((kpi) =>
		linkKpiIds.includes(kpi.id),
	);
}

export function getLinksForGoal(goalId: string): GoalKpiLink[] {
	return (storeState.data?.goalKpiLinks ?? []).filter(
...
export function getAssignmentsByProfile(
	profileId: EvaluationProfile,
): EmployeeAssignment[] {
	return (storeState.data?.assignments ?? []).filter(
		(a) => a.profileId === profileId,
	);
}

export function getAssignmentByEmployee(
	employeeId: string,
): EmployeeAssignment | undefined {
	return (storeState.data?.assignments ?? []).find(
...
function doCategoryWeightsSumTo100(): boolean {
	const sum = (storeState.data?.categories ?? []).reduce(
		(acc, c) => acc + c.weight,
		0,
	);
	...
	const { error: apiError } = await client.POST(
		'/employees/{empId}/categories',
		{
			...
			body: {
				...
				...(category.pillarId ? { pillar_id: category.pillarId } : {}),
			},
		},
	);
	if (apiError)
...
export async function updateCategory(
	id: string,
	updates: Partial<Omit<GoalCategory, 'id'>>,
): Promise<void> {
	const empId = getEmployeeId();
	const { error: apiError } = await client.PUT(
		'/employees/{empId}/categories/{catId}',
		{
			...
			body: {
				...
				...(updates.pillarId ? { pillar_id: updates.pillarId } : {}),
			},
		},
	);
	if (apiError)
...
export async function updateGoal(
	...
	const { error: apiError } = await client.PUT('/goals/{goalId}', {
		...
		body: {
			...
			direction:
				(updates.direction as 'ascendente' | 'descendente') ?? 'ascendente',
...
export async function acceptGoalProposal(
	goalId: string,
	proposalId: string,
	reviewedBy: string,
): Promise<void> {
...
export async function updateKpi(
	...
	const { error: apiError } = await client.PUT('/kpis/{kpiId}', {
		...
		body: {
			...
			description: updates.description ?? '',
			unit:
				(updates.unit as 'porcentaje' | 'moneda' | 'numero' | 'binario') ??
				'numero',
			direction:
				(updates.direction as 'ascendente' | 'descendente') ?? 'ascendente',
...
export async function linkKpiToGoal(
	...
	if (apiError)
		throw new Error(
			(apiError as { error?: { message?: string } })?.error?.message ??
				'Error al vincular KPI',
		);
	await reload(true);
}

export async function unlinkKpiFromGoal(
	goalId: string,
	kpiId: string,
): Promise<void> {
	const { error: apiError } = await client.DELETE(
	...
	if (apiError)
		throw new Error(
			(apiError as { error?: { message?: string } })?.error?.message ??
				'Error al desvincular KPI',
		);
	await reload(true);
}

export async function updateLinkWeight(
	goalId: string,
	kpiId: string,
	...
	storeState.data = {
		...
		goalKpiLinks: (storeState.data?.goalKpiLinks ?? []).map((link) =>
			link.goalId === goalId && link.kpiId === kpiId
				? { ...link, weight }
				: link,
		),
	};
...
export async function addAssignment(
	...
	if (apiError) {
		const errObj = (
			apiError as { error?: { message?: string; details?: string[] } }
		)?.error;
...
export async function deleteAssignment(id: string): Promise<void> {
	...
	storeState.data = {
		...storeState.data!,
		assignments: (storeState.data?.assignments ?? []).filter(
			(a) => a.id !== id,
		),
	};
}
...
export async function unassignGoalFromEmployee(
	employeeId: string,
	goalId: string,
): Promise<void> {
	// No dedicated API endpoint. Local-only for now.
...
export async function recordChangeRequest(
	...
	if (data) {
		storeState.data = {
			...storeState.data!,
			changeRequests: [
				...(storeState.data?.changeRequests ?? []),
				data as unknown as ChangeRequest,
			],
		};
	}
}
				...
				goals: storeState.data!.goals.map((g) =>
					g.id === goalId
						? {
								...g,
								progress,
								progressUpdatedAt: new SvelteDate().toISOString(),
							}
						: g,
				),
			};
...
export function getCategoryComments(categoryId: string): GoalComment[] {
	return (
		storeState.data?.categories.find((c) => c.id === categoryId)?.comments ?? []
	);
}
...
export async function loadAssignmentComments(
	assignmentId: string,
): Promise<void> {
	if (!storeState.data) return;
...
export async function refreshAssignmentComments(
	assignmentId: string,
): Promise<void> {
	if (!storeState.data) return;
	await fetchAndSetAssignmentComments(assignmentId);
...
async function fetchAndSetAssignmentComments(
	assignmentId: string,
): Promise<void> {
	if (!storeState.data) return;
...
export async function addAssignmentComment(
	...
	if (data) {
		const comment = mapToGoalComment(data);
		const existing =
			storeState.data?.assignmentComments.get(assignmentId) ?? [];
		const newMap = new SvelteMap<string, GoalComment[]>(
			storeState.data?.assignmentComments ?? new SvelteMap(),
		);
