...
async function fetchDevEmployees(
  query: string | undefined,
  offset: number,
): Promise<EmployeesBody> {
  ...
  const res = await fetch(`/api/v1/dev/employees?${params.toString()}`, {
    credentials: "include",
  });
...
class EmployeePickerStoreImpl {
  ...
  getOptions = (): EmployeeOption[] => {
    ...
    const activeIdx = current.findIndex(
      (o) => o.value === this.activeEmployeeId,
    );
    if (activeIdx !== -1) {
    ...
    if (this.activeEmployeeLabel) {
      return [
        { value: this.activeEmployeeId, label: this.activeEmployeeLabel },
        ...current,
      ];
    }
...
export function createEmployeePickerStore(opts?: {
  useDev?: boolean;
}): EmployeePickerStoreImpl {
