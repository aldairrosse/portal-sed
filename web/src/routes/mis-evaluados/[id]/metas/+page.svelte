<script lang="ts">
	...
	$effect(() => {
		if (employeeId) {
			...
			client
				.GET('/employees/{empId}', { params: { path: { empId: employeeId } } })
				.then(({ data }) => {
					if (data?.data) {
						employeeData = {
							...
							profileName:
								(data.data as Record<string, string>).profileName ?? '',
						};
					}
				});
		}
...
<div class="flex flex-col gap-6">
	<nav class="breadcrumbs text-sm" aria-label="Navegación">
		<ul>
			<li>
				<a href="/mis-evaluados" class="link link-hover text-base-content/50"
					>Mis evaluados</a
				>
			</li>
			...
			<div class="avatar placeholder">
				...
				>
					<span class="text-sm font-bold"
						>{employeeName.charAt(0).toUpperCase()}</span
					>
				</div>
			</div>
			<div>
				...
				<p
					class="text-sm text-base-content/50 flex items-center gap-1.5 mt-0.5"
				>
					<Briefcase class="w-3.5 h-3.5" />
					{employeeData?.jobTitle ?? 'Empleado'} — {titleCase(
						employeeData?.profileName ?? 'colaborador',
					)}
				</p>
			</div>
		</div>
	...
	{:else}
		...
		<div class="space-y-4">
			...
			{#each categories as cat (cat.id)}
				<div class="card bg-base-100 border border-base-300">
					<div class="card-body p-4">
						...
						{#if cat.description}<p class="text-sm text-base-content/60">
								{cat.description}
							</p>{/if}
						<div class="mt-3 space-y-2">
							{#each getGoalsByCategory(cat.id) as goal (goal.id)}
								<div class="border border-base-200 rounded-lg p-3">
									...
									<div
										class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-base-content/50"
									>
										<span>Unidad: {goal.unit}</span>
										<span>Objetivo: {goal.targetValue}</span>
										{#if goal.baselineValue !== undefined}<span
												>Línea base: {goal.baselineValue}</span
											>{/if}
			...
			{:else}
				<div
					class="card bg-base-100 border border-dashed border-base-300 p-8 text-center"
				>
					<p class="text-sm text-base-content/50">
						Este colaborador aún no tiene categorías registradas.
					</p>
					<a
						href={`/objetivos/asignacion?empId=${employeeId}`}
						class="btn btn-primary btn-sm mt-3">Formular metas</a
					>
				</div>
			{/each}
		</div>
