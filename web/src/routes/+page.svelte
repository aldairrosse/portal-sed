	...
	<div class="grid grid-cols-1 lg:grid-cols-10 gap-6">
		...
		<section
			class="bg-(--color-base) rounded-xl {profile === 'director-general'
				...
				: 'lg:col-span-6'}"
		>
			<h2 class="text-sm font-bold text-base-content px-4 py-3 tracking-wide">
				Ciclo {year}
			</h2>
			<!-- Timeline -->
		...
		{#if profile !== 'director-general'}
			...
			<section class="lg:col-span-4 bg-(--color-base) rounded-xl">
				...
				<div class="grid grid-cols-2 gap-3 px-4 pt-3 pb-6 min-h-40">
					...
					>
						<Icon name="file-check" size={44} class="text-primary flex-1" />
				...
				{#each competenciesByPillar as group, i (group.pilar.id)}
					{#if group.items.length > 0}
						...
						>
							...
							<div>
								<h3 class="text-lg font-normal text-white mb-2">
									{group.pilar.name}
								</h3>
								<ul class="space-y-1">
