<script lang="ts">
	...
	async function saveJWeight() {
		...
		try {
			...
			jError = humanizeError(e, 'Error al guardar ponderación');
		} finally {
			jSaving = false;
		}
	}
	...
	const editInitial = $derived(
		editGoal
			? {
					...
					members: editGoal.members.map((m) => ({
	...
	<div class="flex items-center gap-3 mb-6">
		...
		<div>
			...
			<p class="text-sm text-base-content/60">
				Crear y gestionar metas para tu grupo
			</p>
		</div>
	</div>
	...
	{:else}
		<div class="bg-base-100 border border-base-300 rounded-lg p-4 mb-4">
			...
			<div class="flex items-end gap-4">
				...
				{#if jSaving}<span class="text-xs text-base-content/50">Guardando…</span
					>{/if}
			</div>
		</div>
						...
						{#each qualitativeGoals as goal (goal.id)}
							...
							>
								<div class="flex-1">
									...
									>
										{#if goal.direction === 'ascendente'}<TrendingUp
												class="w-3 h-3"
...
<ConfirmDialog
	...
	title="Eliminar meta compartida"
