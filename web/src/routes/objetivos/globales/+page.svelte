<script lang="ts">
	...
	function openEdit(goal: GlobalGoal) {
		editGoal = goal;
		createKind =
			goal.goal_kind === 'quantitative' ? 'quantitative' : 'qualitative';
		showCreate = true;
	}
	...
	const qualitativeGoals = $derived(
		goals.filter((g) => g.goal_kind === 'qualitative'),
	);
	const quantitativeGoals = $derived(
		goals.filter((g) => g.goal_kind === 'quantitative'),
	);
	...
	const weightedProgress = $derived(
		(() => {
			return Math.min(
				...
				Math.max(
					0,
					goals.reduce((s, g) => s + progressPercent(g) * g.weight, 0) / 100,
				),
			);
		})(),
	);

	// En inicio-anio la barra refleja distribución (totalSum), en avance refleja progreso ponderado
	const displayProgress = $derived(
		phase === 'inicio-anio' ? totalSum : weightedProgress,
	);
	...
	function progressPercent(goal: {
		...
		return calcProgress(
			...
			goal.direction as 'ascendente' | 'descendente',
		);
	}

	const isWeightValid = $derived(
		Math.abs(totalSum - 100) < 0.01 || goals.length === 0,
	);
	...
	<div class="flex items-center gap-3 mb-6">
		...
		<div>
			...
			<p class="text-sm text-base-content/60">
				Crear y asignar metas globales desde RH
			</p>
		</div>
	</div>
	...
	{:else}
		<div class="bg-base-100 border border-base-300 rounded-lg p-4 mb-4">
			...
			<p class="text-xs text-base-content/60 mb-3">
				RH edita Global; Personal = 100 − Global se calcula automáticamente.
			</p>
			...
			<div class="flex items-end gap-4">
				...
				{#if weightSaving}<span class="text-xs text-base-content/50"
						>Guardando…</span
					>{/if}
			</div>
		</div>
		<div class="bg-base-200 rounded-lg p-4 mb-6">
			...
			/>
			{#if !isWeightValid}
				<p class="text-xs mt-2 {isOverWeight ? 'text-error' : 'text-warning'}">
					La suma total de pesos debe ser 100% (actual {totalSum.toFixed(1)}%)
				</p>
			{/if}
		</div>
		...
		<details
			open
			class="collapse collapse-arrow bg-base-100 border border-base-300 rounded-lg mb-4"
		>
			<summary
				class="collapse-title min-h-0 pl-4 pr-8 py-3 flex items-center justify-between gap-2"
			>
				...
				<div class="flex items-center gap-2">
					<span class="text-sm text-base-content/60 mr-4"
						>{qualitativeSum}%</span
					>
				</div>
			</summary>
								...
								<div>
									<p class="font-medium flex items-center gap-2">
										...
										<span class="badge badge-ghost shrink-0"
											>Peso: {goal.weight}%</span
										>
									</p>
									<div
										class="flex flex-wrap items-center gap-x-2 text-sm text-base-content/60"
									>
										...
										<span class="text-xs text-base-content/50"
											>{goal.rules.length} reglas · {goal.assignments.length} asignados</span
										>
									</div>
								</div>
								<div class="flex items-center gap-2">
									{#if canEditProgress}
										<div class="flex items-center gap-1">
											...
											<span class="text-xs text-base-content/60"
												>{progressPercent(goal).toFixed(1)}%</span
											>
