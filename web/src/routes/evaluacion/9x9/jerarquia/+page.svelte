<script lang="ts">
	...
	async function loadJefeEvaluatees(): Promise<void> {
		...
		try {
			...
			jefeError = e instanceof Error ? e.message : 'Error al cargar evaluados';
		} finally {
			jefeLoading = false;
		}
	}
						...
						<div class="card-body p-0">
							...
							<div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
								{#if selectedEmployee.jobTitle}
									<div
										class="flex items-center gap-3 bg-base-200 rounded-lg p-3"
									>
									...
									>
										...
										<div>
											...
											<p class="text-xs font-medium">
												{titleCase(selectedEmployee.profileName)}
											</p>
										</div>
									</div>
								{/if}
										...
										<div>
											...
											{:else if selectedEmployeeScore !== null}
												<p class="text-sm font-medium">
													{Math.round(selectedEmployeeScore)}%
												</p>
											{:else}
				...
				{:else}
					<div class="card bg-base-100 border border-base-300">
						<div class="card-body p-8 text-center">
							<Users class="w-10 h-10 text-base-content/20 mx-auto mb-3" />
							<p class="text-sm text-base-content/40">
								Selecciona un nodo o colaborador del organigrama para ver sus
								detalles.
							</p>
						</div>
					</div>
