...
type OrganizationQuery struct {
	...
	withOrgNodes *OrgNodeQuery
	withCycles   *CycleQuery
...
// QueryOrgNodes chains the current query on the "org_nodes" edge.
func (_q *OrganizationQuery) QueryOrgNodes() *OrgNodeQuery {
...
func (_q *OrganizationQuery) QueryCycles() *CycleQuery {
...
func (_q *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withOrgNodes: _q.withOrgNodes.Clone(),
...
// WithOrgNodes tells the query-builder to eager-load the nodes that are connected to
// the "org_nodes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithOrgNodes(opts ...func(*OrgNodeQuery)) *OrganizationQuery {
	query := (&OrgNodeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOrgNodes = query
	return _q
}
...
func (_q *OrganizationQuery) WithCycles(opts ...func(*CycleQuery)) *OrganizationQuery {
	query := (&CycleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withOrgNodes; query != nil {
		if err := _q.loadOrgNodes(ctx, query, nodes,
			func(n *Organization) { n.Edges.OrgNodes = []*OrgNode{} },
			func(n *Organization, e *OrgNode) { n.Edges.OrgNodes = append(n.Edges.OrgNodes, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCycles; query != nil {
		if err := _q.loadCycles(ctx, query, nodes,
			func(n *Organization) { n.Edges.Cycles = []*Cycle{} },
			func(n *Organization, e *Cycle) { n.Edges.Cycles = append(n.Edges.Cycles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrganizationQuery) loadOrgNodes(ctx context.Context, query *OrgNodeQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrgNode)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orgnode.FieldOrganizationID)
	}
	query.Where(predicate.OrgNode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.OrgNodesColumn), fks...))
	}))
...
func (_q *OrganizationQuery) loadCycles(ctx context.Context, query *CycleQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Cycle)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(cycle.FieldOrganizationID)
	}
	query.Where(predicate.Cycle(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.CyclesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
